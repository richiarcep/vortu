      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <header style={{height:56,background:theme==='dark'?'rgba(11,11,12,.9)':'rgba(251,251,253,.9)',backdropFilter:'saturate(180%) blur(20px)',WebkitBackdropFilter:'saturate(180%) blur(20px)',borderBottom:`.5px solid ${T.hairline}`,display:'flex',alignItems:'center',padding:'0 24px',flexShrink:0,position:'sticky',top:0,zIndex:10}}>
          ...
          <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:28,height:28,borderRadius:999,background:'linear-gradient(135deg,#3D2BFF,#A5B1FF)',color:'#fff',display:'grid',placeItems:'center',fontWeight:600,fontSize:11}}>
                ...
                {PAISES.map(p=>(
                  <div key={p.code} onClick={()=>{if(p.disponible)setPaisSeleccionado(p.code)}}
                    style={{
                      ...
                      background:paisSeleccionado===p.code?'rgba(61,43,255,.06)':p.disponible?T.card:'rgba(0,0,0,.02)',
          ...
          {paso===3&&(
            <Card>
              ...
              <div style={{display:'flex',gap:10,marginBottom:20}}>
                <button onClick={()=>setTieneCert(false)} style={{flex:1,padding:'14px',border:`.5px solid ${!tieneCert?T.blue:T.hairline}`,borderRadius:12,background:!tieneCert?'rgba(61,43,255,.06)':T.card,cursor:'pointer',fontFamily:'inherit',boxShadow:!tieneCert?`0 0 0 1px ${T.blue}`:'none'}}>
                ...
                <button onClick={()=>setTieneCert(true)} style={{flex:1,padding:'14px',border:`.5px solid ${tieneCert?T.blue:T.hairline}`,borderRadius:12,background:tieneCert?'rgba(61,43,255,.06)':T.card,cursor:'pointer',fontFamily:'inherit',boxShadow:tieneCert?`0 0 0 1px ${T.blue}`:'none'}}>
                ...
                <div style={{display:'flex',gap:10}}>
                  {[
                    ...
                    <div key={a.key} onClick={()=>setForm(f=>({...f,ambiente:a.key}))}
                      style={{flex:1,padding:'12px 14px',borderRadius:10,border:`.5px solid ${form.ambiente===a.key?a.key==='produccion'?T.green:T.blue:T.hairline}`,background:form.ambiente===a.key?a.key==='produccion'?T.greenSoft:'rgba(61,43,255,.06)':T.card,cursor:'pointer',boxShadow:form.ambiente===a.key?`0 0 0 1px ${a.key==='produccion'?T.green:T.blue}`:'none'}}>
