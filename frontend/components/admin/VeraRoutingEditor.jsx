...
function TriggerNode({ data }) {
  return (
    <div style={{
      ...
      boxShadow: '0 2px 8px rgba(0,113,227,.1)',
    }}>
      <div style={{
...
function ModelNode({ data }) {
  const colors = {
    claude: { bg: '#F0F7FF', border: '#0071E3', text: '#003D8F' },
    openai: { bg: '#F0FAF4', border: '#10A37F', text: '#0F6E56' },
    gemini: { bg: '#FFF7ED', border: '#EA580C', text: '#9A3412' },
    perplexity: { bg: '#F5F3FF', border: '#7C3AED', text: '#4C1D95' },
  }
...
export default function VeraRoutingEditor({ token }) {
  ...
  const getToken = () => token || (typeof window !== 'undefined' ? localStorage.getItem('nexum_token') : null)
                ...
                {rules.map(r => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10.5, color: T.text4, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      {r.models?.map(m => (
                        <span key={m} style={{ padding: '1px 6px', background: '#EBF5FF', color: '#0071E3', borderRadius: 3 }}>
                          {m}
                        </span>
                      ))}
                    </div>
              ...
              {activeRule ? (
                ...
                >
                  ...
                  <MiniMap nodeColor={n => {
                    ...
                    if (n.type === 'strategy') return '#7C3AED'
              ...
              <tbody>
                {logs.map(l => (
                  <tr key={l.id} style={{ borderBottom: `.5px solid ${T.hairline}` }}>
                    ...
                    <td style={{ padding: 8 }}>
                      <span style={{ padding: '1px 6px', background: '#EBF5FF', color: '#0071E3', borderRadius: 3, fontSize: 10 }}>
                        {l.winning_model || '—'}
                      </span>
                    </td>
