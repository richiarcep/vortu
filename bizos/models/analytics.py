...
class ProspectorSearch(Base):
    ...
    location     = Column(String(200), default="Madrid")
    status       = Column(String(20), default="pending")  # pending | running | done | error
    total_leads  = Column(Integer, default=0)
    leads_contactar = Column(Integer, default=0)
    created_at   = Column(DateTime, default=datetime.utcnow)
...
class ProspectorLead(Base):
    ...
    id                = Column(Integer, primary_key=True, index=True)
    search_id         = Column(Integer, ForeignKey("prospector_searches.id"), nullable=False, index=True)
    source            = Column(String(50))        # google_maps | instagram | facebook | linkedin
    ...
    score             = Column(Float, default=5)   # 1-10 score de relevancia IA
    razon_score       = Column(Text)
    canal_recomendado = Column(String(50))
    tipo_negocio      = Column(String(200))
    pain_point        = Column(Text)
    contactar         = Column(Boolean, default=True)
    mensaje_generado  = Column(Text)
    estado            = Column(String(30), default="pendiente")  # pendiente | aprobado | enviado | descartado
