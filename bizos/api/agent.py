...
def get_resumen(
    ...
    ingresos = float(db.execute(text(
        "SELECT COALESCE(SUM(monto),0) FROM registro_diario WHERE company_id=:cid AND tipo='ingreso' AND fecha >= :fecha"
    ), {"cid": cid, "fecha": last_30}).scalar() or 0)

    gastos = float(db.execute(text(
        "SELECT COALESCE(SUM(monto),0) FROM registro_diario WHERE company_id=:cid AND tipo='gasto' AND fecha >= :fecha"
    ), {"cid": cid, "fecha": last_30}).scalar() or 0)
    ...
    return {
        ...
        "saldo_caja_actual": round(ingresos - gastos, 2),
