...
// ── Edita aquí qué módulos tiene cada plan ─────────────────────────────────────
const PLAN_MODULES = {
  starter:  ['dashboard', 'contabilidad', 'ventas', 'clientes'],
  pro:      ['dashboard', 'contabilidad', 'finanzas', 'hr', 'ventas', 'clientes', 'documentos', 'agente', 'marketing'],
  business: ['dashboard', 'contabilidad', 'finanzas', 'hr', 'proyectos', 'ventas', 'clientes', 'documentos', 'agente', 'marketing'],
  admin:    'all',
}
...
export default function Sidebar({ active }) {
  ...
  const allowedModules = isAdmin ? 'all' : (PLAN_MODULES[plan] || PLAN_MODULES.starter)
  ...
  return (
    <div style={{ width: '220px', minWidth: '220px', background: NAVY, height: '100vh', flexShrink: 0, display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, alignSelf: 'flex-start' }}>
      ...
      <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            {isAdmin ? 'Nexum Admin' : `Plan ${plan || 'Starter'}`}
          </div>
          {!isAdmin && (
            <a href="/pricing" style={{ fontSize: '9px', color: CYAN, textDecoration: 'none', fontWeight: '700' }}>↑ Upgrade</a>
          )}
        </div>
        {!isAdmin && (
          <>
            <div style={{ height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', overflow: 'hidden' }}>
              <div style={{ width: plan === 'business' ? '95%' : plan === 'pro' ? '68%' : '30%', height: '100%', background: `linear-gradient(90deg, ${CYAN}, ${BLUE})`, borderRadius: '2px' }} />
            </div>
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginTop: '5px' }}>
              {plan === 'business' ? '10 módulos activos' : plan === 'pro' ? '9 módulos activos' : '4 módulos activos'}
            </div>
          </>
        )}
      </div>
