...
function Sidebar({ active }) {
  const links = [
    ...
    { label: 'Contabilidad',     href: '/contabilidad', icon: '📒' },
    { label: 'Finanzas',         href: '/finanzas',     icon: '💰' },
    { label: 'Recursos Humanos', href: '/hr',           icon: '👥' },
    { label: 'Proyectos',        href: '/proyectos',    icon: '📋' },
    { label: 'Clientes',         href: '/clientes',     icon: '💬' },
    { label: 'Ventas',           href: '/ventas',       icon: '🛒' },
    { label: 'Documentos',       href: '/documentos',   icon: '📁' },
    { label: 'Agente IA',        href: '/agente',       icon: '🤖' },
    { label: 'Marketing',        href: '/marketing',    icon: '📣' },
  ]
  return (
    <div style={{ width: '220px', background: NAVY, minHeight: '100vh', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '24px 20px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
          <div style={{ width: '32px', height: '32px', background: 'white', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="17" height="17" viewBox="0 0 20 20" fill="none"><path d="M4 16V4L16 16V4" stroke="#0B1426" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
          <div>
      ...
      <nav style={{ flex: 1, padding: '4px 0' }}>
        {links.map(item => {
          ...
          return (
            <a key={item.href} href={item.href} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 20px', color: isActive ? 'white' : 'rgba(255,255,255,0.4)', textDecoration: 'none', fontSize: '13px', fontWeight: isActive ? '600' : '400', background: isActive ? 'rgba(255,255,255,0.08)' : 'transparent', borderLeft: isActive ? '2px solid white' : '2px solid transparent', transition: 'all 0.15s' }}>
      ...
      <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>Vortu Pro</div>
        <div style={{ height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', overflow: 'hidden' }}>
          <div style={{ width: '68%', height: '100%', background: `linear-gradient(90deg, ${CYAN}, ${BLUE})`, borderRadius: '2px' }} />
        </div>
        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginTop: '5px' }}>68% del plan utilizado</div>
      </div>
    </div>
  )
}
...
function SettingsInner() {
  ...
  useEffect(() => {
    ...
    if (success) { setSaved(true); setTimeout(() => setSaved(false), 4000) }
  }, [])
  ...
  async function handleUpgrade(planId) {
    ...
    try {
      const res = await fetch(`${API}/api/billing/license/checkout`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ plan_id: planId }) })
    ...
    <div style={{ minHeight: '100vh', background: '#f4f6fb', display: 'flex', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
          ...
          {saved && (
            <div style={{ padding: '12px 16px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '10px', marginBottom: '16px', color: GREEN, fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px', animation: 'fadeUp 0.3s ease' }}>
                ...
                <>
                  ...
                  {billingStatus && billingStatus.plan !== 'none' && (
                    <div style={{ background: NAVY, borderRadius: '16px', padding: '24px 28px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '22px', fontWeight: '800', color: 'white', letterSpacing: '-0.5px', marginBottom: '4px' }}>Vortu {planCfg.name}</div>
                      ...
                      <div style={{ display: 'flex', gap: '10px' }}>
                        ...
                        {!billingStatus.cancel_at_period_end && billingStatus.status === 'active' && (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '20px' }}>
                    {[
                      ...
                      return (
                        <div key={plan.id} style={{ ...card, padding: '22px', border: isCurrent ? `2px solid ${plan.color}` : '1px solid #e5e9f0', position: 'relative', background: isCurrent ? `${plan.color}08` : 'white' }}>
                          ...
                          <div style={{ fontSize: '16px', fontWeight: '800', color: plan.color, marginBottom: '4px' }}>{plan.name}</div>
                          ...
                          <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '16px' }}>+ €{plan.license} licencia única</div>
                          ...
                          <button onClick={() => !isCurrent && handleUpgrade(plan.id)} disabled={isCurrent || upgradeLoading === plan.id} style={{ width: '100%', padding: '10px', borderRadius: '9px', border: isCurrent ? 'none' : `1.5px solid ${plan.color}`, background: isCurrent ? `${plan.color}15` : 'white', color: isCurrent ? plan.color : plan.color, fontWeight: '700', fontSize: '13px', cursor: isCurrent ? 'default' : 'pointer', fontFamily: 'inherit', marginTop: '14px', transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                            {upgradeLoading === plan.id ? <><span style={{ width: '14px', height: '14px', border: '2px solid rgba(0,0,0,0.2)', borderTopColor: plan.color, borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} />Redirigiendo...</> : isCurrent ? 'Plan actual' : `Cambiar a ${plan.name}`}
                          </button>
                        </div>
                      )
                    })}
