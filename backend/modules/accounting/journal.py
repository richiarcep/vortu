...
class Account(Base):
    ...
    The account code is unique PER COMPANY (multi-tenant), not globally, so two
    companies (and two countries' charts) can both use e.g. code "100".
    The actual chart installed depends on the company's country (see
    setup_chart_of_accounts + country.registry).
    """
    ...
    code = Column(String, nullable=False, index=True)   # unique per company, not global
...
def setup_chart_of_accounts(db, company_id: int, country: str = None):
    """
    Creates the chart of accounts for a new company, using the official chart for
    the company's country (country.registry). Falls back to the generic
    DEFAULT_ACCOUNTS only if the country is unknown/unsupported.
    Idempotent: does nothing if the company already has accounts.
    """
    ...
    if country:
        try:
            from country.registry import get_chart_of_accounts
            accounts = get_chart_of_accounts(country)
        except (ValueError, Exception):
            accounts = DEFAULT_ACCOUNTS  # unknown country → generic fallback
